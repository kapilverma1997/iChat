...
export function initSocket(server) {
  ...
  // Configure CORS to allow connections from multiple origins
  ...
  globalForSocket.io.on('connection', (socket) => {
    ...
    socket.on('authenticate', async (token) => {
      ...
      try {
        ...
        if (decoded && decoded.userId) {
          ...
          // Publish user online status to RabbitMQ
          try {
            const { publishPresenceUpdate } = await import('./messageProducer.js');
            const { setUserOnline } = await import('./messageConsumer.js');
            ...
            console.error('Error publishing presence update:', presenceError);
          }
        }
      } catch (error) {
    ...
    socket.on('disconnect', async () => {
      ...
      if (userId) {
        ...
        // Publish user offline status to RabbitMQ
        try {
          const { publishPresenceUpdate } = await import('./messageProducer.js');
          const { setUserOffline } = await import('./messageConsumer.js');
          ...
          console.error('Error publishing offline status:', presenceError);
        }
      }
    });
  });
